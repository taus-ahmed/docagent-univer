...
### Writer + prompt fixes from production jobs 149-151 (2026-07-05)

Generic fixes confirmed by production tests (all verified with synthetic writer tests + py_compile + tsc):

- **A1 — semantic column matching** (`_write_layout_excel._match_group` stage 3): after exact and substring matching, unmatched extracted sections are matched to `binding_column_groups` by **synonym-canonicalized token Jaccard** (≥0.6): `long-term/longterm ↔ non current ↔ noncurrent`, `stockholders/owners ↔ shareholders`, `fixed asset → noncurrent asset`, plural folding, stop-words `total/and/of/amount`. "Long-term Liabilities" now lands in the "Non current liabilities" group's C/D columns instead of A/B overflow. Truly unmatched sections still go to A/B overflow (no data dropped).
- **A2 — content-signature dedup** (`_write_layout_excel`): before group matching, any extracted section whose (label, value) row content is **identical** to an already-kept section is dropped (log: `dropped section 'X' — identical content to 'Y'`). The older superset-dedup required `len(r1) > len(r2)` so exact duplicates were never caught.
- **A4 — gap rows** (`_calculate_layout`): each band now computes `content_end` (last data row or its own totals row directly below) and gets `gap=1` when the next band's header would sit adjacent (template has no spacer row). The row-shift map applies `overflow` below `template_end_row` and `gap` below `content_end_row` — so a totals row stays attached to its section while the next section header moves down. Templates that already have spacer rows are unchanged (gap=0).
- **A5/A7 — multi-document responses** (`extractor.py _unwrap_all`): `_run_cbm_extraction` and `_run_field_extraction` now process **every** entry in a Gemini `documents[]` response (one `DocumentExtractionResult` per doc, filenames `stem_docN.pdf`), instead of `_unwrap` silently discarding `documents[1:]` — the cause of a two-invoice file exporting only the first invoice. Top-level `extracted_fields`/`table_rows` are merged into doc 0 when it lacks them. `_build_cbm_prompt` now instructs Gemini to return a `documents` array for multi-document files and adds a TABLE COMPLETENESS rule (one row object per line item). `_write_cbm_tables` assigns each row to exactly ONE table (by `_table_source`, else best column overlap, else table 0 — the old per-table filter dropped rows matching neither table) and always logs `written/total`.
- **A6 — no phantom columns**: `_find_template_dimensions` now counts only CONTENT cells (non-empty value, extractTarget, or merge) — the spreadsheet editor keeps style-only and cleared `{value:""}` cells which inflated the box to the 50×26 canvas. All writers (form/mixed/table/layout/cbm/fallback) skip cells outside the content bounding box and never assign `""` (which materializes None columns in the export).
- **A3 — value/label misassignment guards** (job 150: "Current Portion of Long-term Debt" = 499200, the section total). The prod DB wasn't reachable locally for the layout_sections dump, so guards were added at both ends: `_build_section_prompt` always forbids the section TOTAL/SUBTOTAL as a row (even when no total cell is bound) and adds a PAIRING rule (a row's value must come from the same document line as its label); `_validate_extraction` step 2 flags any line item whose value **equals the section total** (`row value equals section total` → needs_review).
- **B1 — FIELD SEPARATION RULE** in `_build_vision_prompt` (rule 8) and `_build_cbm_prompt` (rule 6): one piece of information per cell — never company name + address together.
- **B2 — company-name cells** (`_build_cbm_prompt`): any extract cell with `data_type` containing company/organization gets a COMPANY NAME CELLS block ("extract the ORGANIZATION, not the contact person — 'Bill to' is a company name").
- **C1 — no-template extraction enabled** (`extract/page.tsx`): removed `!selectedTemplate` from the Extract button's `disabled` — `handleExtract` already routes to the existing confirmation modal, whose copy now reads "The AI will extract all content from this document in a structured format." Confirm → `startExtract(undefined)` → backend unguided path.

### Complete correctness pass — 16 fixes (2026-07-06)

All verified by `verify_all.py`-style synthetic tests (8 mandatory checks V-S1..V-E6 + bonus S4/S5/E1/E4/E5), py_compile, and tsc. Structural group:

- **S1 — total cells by total-label scan** (`compute_binding_map`): a group's `total_cells` is the value cell of the first row BELOW the band whose **label-column text contains a total indicator** (total/sum/subtotal/…); the first non-empty label below the band decides (total row vs next section header ⇒ no total). Total-label rows are also **bucket delimiters** — blank rows below a "Total …" row are spacers, not data (previously they stretched `end_row` past the total row and the group got NO total). `_run_all_extractions` now takes the L2 `total_cell` from `grp["total_cells"][0]` — the old window scan picked ANY value_target in `start_row..end_row+2`, i.e. labeled DATA rows inside the band (that's how B5 was used as a "total" instead of B9).
- **S2 — static cells never overwritten** (`_write_layout_excel` step 3): before writing an `extracted_fields` value, the target's template cell is checked — non-empty value without extractTarget, or a merge child ⇒ `SKIP extracted_fields <ref> -> static template cell` log, cell untouched.
- **S3 — gap rows**: `_calculate_layout` band `gap` (2026-07-05) verified — 1 blank row before the next section header whenever the template has no spacer.
- **S4 — overflow to parallel columns**: an extracted section matching NO group (even semantically) is placed into an **empty group whose `parallel_group_id` partner already has content** (the C/D half of a matched A/B band), ranked by semantic score; only truly orphaned sections fall to the A/B overflow area.
- **S5 — L2 section truncation** (`_extract_section`): rows > `item_count * 1.5` are truncated to `item_count` (log `[L2] section '…' truncated N -> M rows`) — stops one section absorbing the next one's items.
- **V1 — per-group total cross-validation** (`_validate_extraction(column_groups=…)`): each section's item sum is compared ONLY against its OWN group's `total_cells[0]` (exact→fuzzy label match); sections with no total cell are skipped. The old logic compared against any extracted_field in the same column letter (Current assets vs B11 = Current LIABILITIES' total — guaranteed false positive).

CBM / labeled group: **L1** — `_build_cbm_prompt` appends semantic alias notes per cell via `_CBM_FIELD_ALIASES` (contract/reference/PO ⇒ "May appear as: PO Number, PO Ref, Purchase Order, …"; company_name; invoice_number), matched on data_type or label keywords. **L2** — `_write_cbm_tables` resets every table-row cell to default `Alignment()` so all rows render consistently. **L3** — bare-currency retry (`_retry_bare_currency` + `_CBM_MODEL=gemini-2.5-flash`) verified.

No-template: **N1** — download buttons no longer require a template: history shows them for `status==="completed" && successful>0` (the `jobHasTemplate` gate and "No template used" placeholder removed — note: this also removed partial downloads for *failed* jobs, per spec `completed AND NOT failed`); extract page uses `isDone && !isFailed`. No-template exports go through `_write_flat_table` (already worked).

Edge cases: **E1** — `_validate_extraction` returns early for `scanned_pdf`/`image`/text<50 chars: no text cross-validation, all confidences medium, `needs_review=True`, note "Scanned document — manual verification recommended". **E2** — verified: `single_document=True` only on the structural path; CBM/field paths process every `documents[]` entry, unguided keeps L1's multi-doc map. **E3** — verified: create + update (grid changed) both call `_compute_and_store_cbm`; update logs `[TEMPLATE] CBM regenerated on template update`. **E4** — Layer 1 uses only the first 20 pages of larger files (`_L1_MAX_PAGES`); Layer 2 always uses each section's own page. **E5** — `_llm_json` exponential backoff on 429/rate-limit/quota errors: 1s → 2s → 4s… cap 30s, log `[RATE] 429 received — backing off Xs`; non-429 failures keep the fixed 2s retry. **E6** — a layout template with ZERO content cells (no values, no extract targets) routes to unguided extraction (`[ROUTE] … EMPTY TEMPLATE (0 content cells) -> unguided`), never the field prompt.
