...
**Routing decided at the START, by template type (2026-06-26).** `run_extraction` preprocesses, computes the binding map once, then routes deterministically into one of three named paths — so the right extraction runs from the beginning (no late/export-time correction):
- `binding_map is None` (no template) → **`_run_unguided_extraction`** → three layers with no template → two-column `A`/`B` `layout_sections` + label-keyed `extracted_data` → flat writer.
- `binding_map.has_table_data is True` (structural template — column headers + blank rows) → **`_run_layout_extraction`** → three layers → `layout_sections` → layout writer.
- `binding_map.has_table_data is False` (labeled template — explicit fields) → **`_run_field_extraction`** → ONE template-guided call (`_build_vision_prompt` + `_process_vision_result`), **no Layer 1/2/3** → `extracted_fields` → form/mixed/table writer.
