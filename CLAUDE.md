...
**Mode handling:** layout templates (`binding_map.has_table_data`) and no-template files run the full three layers; **field-mode labeled templates reuse the proven single-call path** (`_build_vision_prompt` + `_process_vision_result`) to guarantee `extracted_fields` compatibility with the form writer. Multi-document files yield one `DocumentResult` per document. Logging: `[L1]`/`[L2]`/`[L3]`/`[RESULT]`. Verified: live-Gemini layout e2e (assets A/B + liabilities C/D), live field-mode e2e (`extracted_fields` B2..B7), and deterministic mocked tests for multi-doc, scanned validation, completeness, and financial cross-validation.
